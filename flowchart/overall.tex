...
\node (check_obj) [decision, below of=rotate, yshift=-1.5cm, text width=2cm] {Check if this object is another robot or wall};

\node (no1) [io, below of=check_obj, yshift=-2cm, text width=4cm] {Pass the target coordinate to motor controller};
\node (yes1) [process, right of=check_obj, xshift=4cm] {Choose next cloest target};
...
\node (home) [process, below of=check_num, yshift=-2cm] {Return to starting point};
